...
class ParseFilter {
	...
	function BB_Parse($source, $use_html = TRUE) {
		...
		if ($count_start AND $count_start == $count_end) {
			...
			$source = str_ireplace( "[quote]", "<!--QuoteBegin--><div class=\"quote\"><!--QuoteEBegin-->", $source );
			$source = preg_replace( "#\[quote=(.+?)\]#i", "<!--QuoteBegin \\1 --><div class=\"title_quote\">{$lang['i_quote']} \\1</div><div class=\"quote\"><!--QuoteEBegin-->", $source );
			$source = str_ireplace( "[/quote]", "<!--QuoteEnd--></div><!--QuoteEEnd-->", $source );
		...
		if ($count_start AND $count_start == $count_end) {
			...
			$source = preg_replace_callback( "#\[spoiler\]#i", array( &$this, 'build_spoiler'), $source );
			$source = preg_replace_callback( "#\[spoiler=(.+?)\]#i", array( &$this, 'build_spoiler'), $source );
			$source = str_ireplace( "[/spoiler]", "<!--spoiler_text_end--></div><!--/dle_spoiler-->", $source );
	...
	function decodeBBCodes($txt, $use_html = TRUE, $wysiwig = false) {
		...
		$txt = preg_replace_callback( "#<!--dle_leech_begin--><a href=[\"'](http://|https://|ftp://|ed2k://|news://|magnet:)?(\S.+?)['\"].*?" . ">(.+?)</a><!--dle_leech_end-->#i", array( &$this, 'decode_leech'), $txt );
		...
		if( !$wysiwig ) {
			...
			$txt = preg_replace_callback( "#<noindex><a href=[\"'](http://|https://|ftp://|ed2k://|news://|magnet:)?(\S.+?)['\"].*?" . ">(.+?)</a></noindex>#i", array( &$this, 'decode_url'), $txt );
