  ...
  <div class="box-content">
    ...
    <table class="table table-normal table-hover">
      ...
      <tr>
        ...
        <td style="width: 180px">{$lang['user_reg']}</td>
        <td style="width: 180px">{$lang['user_last']}</td>
        <td style="width: 40px">{$lang['user_news']}</td>
        <td style="width: 40px">{$lang['user_coms']}</td>
...
HTML;
	...
	while ( $row = $db->get_row() ) {
		...
		if( $row[news_num] == 0 ) {
			...
			$news_link = "<a class=\"status-info\" href=\"{$config['http_home_url']}index.php?subaction=allnews&user=" . urlencode( $row['name'] ) . "\" target=\"_blank\">" . $row[news_num] . "</a>";
		}
		if( $row[comm_num] == 0 ) {
			...
			$comms_link = <<<HTML
				...
				<a href="#" target="_blank" data-toggle="dropdown" data-original-title="{$lang['edit_com']}" class="status-info tip"><b>{$row['comm_num']}</b></a>
				  <ul class="dropdown-menu text-left">
				   ...
				   <li class="divider"></li>
				   <li><a onclick="javascript:cdelete('{$row['user_id']}'); return(false)" href="?mod=editusers&action=dodelcomments&user_hash={$dle_login_hash}&id={$row['id']}"><i class="icon-trash"></i> {$lang['comm_del']}</a></li>
				  </ul>
				</div>
HTML;
		}
